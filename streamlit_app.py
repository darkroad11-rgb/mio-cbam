...
def pulisci_numero(val):
    """Gestisce virgole, valori #VALUE! e celle vuote."""
    if pd.isna(val) or str(val).strip() in ["", "#VALUE!", "nan", "NaN"]:
        return np.nan
    if isinstance(val, str):
        val = val.strip().replace(",", ".")
        # Rimuove tutto tranne numeri, punto e segno meno
        val = re.sub(r'[^\d.-]', '', val)
        try:
            return float(val)
        except:
...
def estrai_rotta(val):
    """Estrae solo le lettere tra parentesi (es. (C) da (C)(1))."""
...
def load_data():
    ...
    if not os.path.exists(f_bench) or not os.path.exists(f_def):
        st.error("File CSV non trovati. Controlla i nomi dei file caricati.")
        st.stop()
...
if len(df_valido) > 1:
    ...
    for _, r in df_valido.iterrows():
        label = estrai_rotta(r[col_ind_bench])
        # Evita duplicati se lo stesso indicatore appare per errore
...
# 2. EMISSIONI (Fallback Other Countries)
if usare_reali:
    ...
    # FALLBACK AUTOMATICO verso 'Other Countries and Territories'
    if pd.isna(val_p):
        row_o = defaults[(defaults[HS_D] == codice_hs) & (defaults[COL_PAESE].str.contains("Other Countries", na=False))]
        emiss_finali = pulisci_numero(row_o[col_yr].iloc[0]) if not row_o.empty else 0.0
        tipo_info = "Default (Other Countries)"
        st.warning(f"Dati per {paese_sel} assenti. Applicato valore globale.")
    else:
        emiss_finali = val_p
